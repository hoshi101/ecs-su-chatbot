...
# Import API keys from config - Updated for new structure
...
@tool
def rag_search_tool(query: str, similarity_threshold: float = 0.7) -> str:
    """Top-K chunks from KB with similarity threshold support (empty string if none)"""
    try:
        from src.backend.services.vectorstore import _get_qdrant_client, embeddings, QDRANT_COLLECTION_NAME
        from langchain_qdrant import QdrantVectorStore

        # Get vectorstore directly to configure search parameters
        ...
        # Use similarity threshold if less than 1.0, otherwise use regular search
        ...
        # Fallback to original retriever if threshold configuration fails
        try:
            retriever_instance = get_retriever()
            docs = retriever_instance.invoke(query)
            return "\n\n".join(d.page_content for d in docs) if docs else ""
        except Exception as fallback_e:
            return f"RAG_ERROR::{e} (Fallback failed: {fallback_e})"
...
- User: "Hello there!" -> Route: 'end', reply='Hello! I'm {BOT_NAME}, your {DOMAIN_NAME} assistant. How can I help you today?'"""
    ...
    # NEW LOGIC: Override router decision if force web search is enabled
    if force_web_search and web_search_enabled:
        if result.route in ["rag", "answer"]:  # Override RAG/answer to force web search
            result.route = "web"
            router_override_reason = "User forced web search; redirected to web search."
            print(f"Router decision overridden: changed from '{initial_router_decision}' to 'web' because user forced web search.")
    # EXISTING LOGIC: Override router decision if web search is disabled and LLM chose 'web'
...
def rag_node(state: AgentState) -> AgentState:
    ...
    # Step 3: Retrieve documents using enhanced query with similarity threshold
    chunks = rag_search_tool.invoke({"query": query, "similarity_threshold": similarity_threshold})
